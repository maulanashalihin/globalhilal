...
export const config = {
	...
	rateLimit: {
		...
		/** Looser per-IP limit for the public read-only API (generous for
		 *  masjid sites/apps, bounded against scrapers). */
		apiMax: Number(pick(process.env.RATE_LIMIT_API_MAX, "600")),
		apiWindow: Number(pick(process.env.RATE_LIMIT_API_WINDOW, "60")),
	},
