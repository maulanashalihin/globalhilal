...
export interface RateLimitOptions {
	 ...
	 */
	paths?: string[];
	/** When set, enforce for any path starting with one of these prefixes. */
	prefixes?: string[];
}
...
/** Best-effort client IP (X-Forwarded-For first entry, else peer IP, else 'local'). */
export function clientIp(c: Context<AppEnv>): string {
	const forwarded = c.req.raw.headers.get("x-forwarded-for");
	if (forwarded) return forwarded.split(",")[0]!.trim();
	const server = c.env as unknown as BunServer | undefined;
	return server?.requestIP?.(c.req.raw)?.address ?? "local";
}
...
export function rateLimit(opts: RateLimitOptions) {
	...
	return async (c: Context<AppEnv>, next: Next) => {
		// Path filter — only enforce for the configured paths/prefixes (see RateLimitOptions).
		if (opts.paths || opts.prefixes) {
			const pathname = new URL(c.req.url).pathname;
			const hitPaths = opts.paths?.includes(pathname) ?? false;
			const hitPrefixes = opts.prefixes?.some((p) => pathname.startsWith(p)) ?? false;
			if (!hitPaths && !hitPrefixes) return next();
		}
		const now = Date.now();
		const key = clientKey(
			c.req.raw,
