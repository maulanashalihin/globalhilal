<script lang="ts">
  ...
  const inputClass =
    'w-full px-3 py-2.5 border border-gh-line rounded-lg bg-gh-sky text-gh-ink text-[0.95rem] focus:outline-2 focus:outline-gh-gold focus:-outline-offset-1 focus:border-gh-gold'

  const btnPrimary =
    'inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-gh-ink rounded-lg bg-gh-ink text-gh-sky font-semibold text-sm cursor-pointer transition-colors hover:opacity-85 hover:no-underline disabled:opacity-60 disabled:cursor-not-allowed'
</script>
  ...
  <Layout>
    ...
    <div class="grid grid-cols-[300px_1fr] gap-5 items-start max-md:grid-cols-1">
      <aside class="flex flex-col gap-4">
        <section
          class="bg-gh-panel border border-gh-line rounded-lg p-6 flex flex-col items-center text-center gap-2"
        >
          {#if user.avatarUrl}
            <img
          ...
          {:else}
            <span
              class="inline-flex items-center justify-center w-11 h-11 rounded-full bg-gh-gold text-gh-night text-sm font-bold shrink-0"
              aria-hidden="true"
            >
              {user.name
                .split(/\s+/)
          ...
          <p class="text-gh-soft m-0">{user.email}</p>
          <div class="flex items-center justify-center gap-2 flex-wrap">
            <span
              class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold capitalize bg-gh-gold-soft text-gh-gold"
            >
              {user.role}
            </span>
          ...
          <div class="flex flex-col items-center gap-2 w-full mt-3">
            ...
            {#if selectedName}
              <span class="text-gh-soft text-sm">{selectedName}</span>
            {/if}
            {#if message}
            ...
            {#if phase === 'uploading'}
              ...
              >
                <div
                  class="h-full rounded-full bg-gh-gold transition-[width] duration-[120ms] ease-out"
